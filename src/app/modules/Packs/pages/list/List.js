...
import {fetchPacks, packsActions, usePacksState} from "../../../../../redux/packs";
import {Loader} from "../../../../../_ae/components/loader";

export const List = () => {
  const dispatch = useDispatch();
  const {data, isLoading, metadata} = usePacksState();
  const {pagination, filters, sort} = metadata;
  const {setPage, setPerPage} = packsActions;

  useEffect(()=>{
    dispatch(fetchPacks(metadata));
  }, [
    pagination.page, pagination.perPage, sort.field, sort.asc,
    filters.search, filters.variant, filters.priceRange
  ])
